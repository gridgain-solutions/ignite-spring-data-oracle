...
@RepositoryConfig(cacheName = INVENTORY_CACHE)
@Repository
public interface InventoryRepository extends IgniteRepository<Inventory, Integer> {

    @Query("SELECT * FROM INVENTORY WHERE INVENTORY_ID = ?")
    Cache.Entry<Integer, Inventory> findById(int id);
}