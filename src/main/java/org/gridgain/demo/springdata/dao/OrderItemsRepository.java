...
@RepositoryConfig(cacheName = ORDER_ITEMS_CACHE)
@Repository
public interface OrderItemsRepository extends IgniteRepository<OrderItems, Integer> {

    @Query("SELECT * FROM ORDER_ITEMS WHERE ORDER_ITEMS_ID = ?")
    Cache.Entry<Integer, OrderItems> findById(int id);
}